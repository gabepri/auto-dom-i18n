...
- **listOption.ts / defaults.ts** — Every list-valued config option follows one convention (`ListOption<T> = T[] | ((defaults: T[]) => T[])`): a plain array is *unioned* with the library defaults (deduped), a function gets the defaults and its return value is used verbatim. All of them route through the single `resolveListOption` helper, and each default list is an exported public constant (`DEFAULT_IGNORE_SELECTORS`, …). **Any new list option must use this helper** — never add an `extra*`/`inherit*` variant or a per-option merge flag. `DEFAULT_IGNORE_SELECTORS` is a curated list that includes browser-extension UI containers (1Password/LastPass/Grammarly), whose localized UI would otherwise be reported as missing source copy; it's expected to grow, which is exactly why plain arrays inherit rather than replace.
...
- **Bundle size: `npm run size`** (in CI after `npm run build`) gates the gzipped dist, measured on a *minified* copy rather than the shipped file. The published ESM keeps its comments and a consumer's bundler strips them, so gating the shipped form priced JSDoc (38% of the old number) instead of code. Comments are free here; new code is not. `intl-messageformat` is external and not counted; consumers still pay for it.
